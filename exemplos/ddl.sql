...
-- essa função é um código que peguei emprestado de um exercício que passei para outra pupila minha
-- tem algumas coisas que poderiam ser melhores nessa função, mas dá para você ter uma noção geral
CREATE OR REPLACE FUNCTION exercise.validate_cpf(cpf_input TEXT)
 RETURNS BOOLEAN AS $$
 DECLARE
...
CREATE TABLE IF NOT EXISTS exercise.user (
  id INTEGER NOT NULL GENERATED ALWAYS AS IDENTITY,
  
  cpf VARCHAR(11) NOT NULL,
  age INTEGER     NOT NULL,

  CONSTRAINT exercise_user_pk     PRIMARY KEY (id),
  CONSTRAINT exercise_user_uq_cpf UNIQUE      (cpf),

  -- exemplo de CK complexa sendo criada com base numa função
  CONSTRAINT exercise_user_ck_cpf CHECK (
    exercise.validate_cpf(cpf)
  ),
  
  -- exemplo de CK simples sendo criada diretamente
  CONSTRAINT exercise_user_ck_age CHECK (
    age >= 10 AND age <= 100
  )
);
