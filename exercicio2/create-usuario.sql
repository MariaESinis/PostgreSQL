...
CREATE TYPE exercicio2.status AS ENUM ('ativo', 'inativo', 'bloqueado', 'pendente');
...
CREATE OR REPLACE FUNCTION exercicio2.fn_valida_nome(IN p_nome TEXT)
RETURNS BOOLEAN 
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
CREATE OR REPLACE FUNCTION exercicio2.fn_valida_telefone(IN p_tel TEXT) 
RETURNS BOOLEAN 
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
-- Funcao valida email
CREATE OR REPLACE FUNCTION exercicio2.fn_valida_email(IN p_email TEXT)
RETURNS BOOLEAN 
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
CREATE TABLE IF NOT EXISTS exercicio2.usuario(
    ...
    status          exercicio2.status   NOT NULL    DEFAULT 'ativo',
